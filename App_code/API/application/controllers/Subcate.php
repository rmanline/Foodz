    ...
    public function getById(){
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Subc_model->getById($_POST['id']);
                if($result != null){
    ...
    public function getByCId(){
            ...
            if($auth && $auth == $this->config->item('encryption_key')){
                ...
                if(isset($data) && !empty($data)){
                    ...
                    $result = $this->Subc_model->getByCId($_POST['id']);
                    if($result != null){
                        echo $this->json->response($result,$this->_OKmessage,$this->_statusOK);
                    }else{
                        echo $this->json->response($this->db->error(),$this->_Errmessage,$this->_statusErr);
    ...
    public function editList(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Subc_model->editList($_POST,$_POST['id']);
    ...
    public function save(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Subc_model->saveList($_POST);
                if($result != null){
                    $id = $this->db->insert_id();
                    $data = $this->Subc_model->getByIdValue($id);
                    echo $this->json->response($data,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response(['error'=>'Something Went Wrong.'],$this->_Errmessage,$this->_statusErr);
                }
    ...
    public function deleteList(){
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Subc_model->deleteList($_POST['id']);
                if($result != null){
                    echo $this->json->response($result,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response(['error'=>'Something Went Wrong.'],$this->_Errmessage,$this->_statusErr);
