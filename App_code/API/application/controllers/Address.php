...
class Address extends CI_Controller{
    ...
    public $_table_column_array = ['uid','address','house','landmark','lat','lng','title','pincode'];
    public $_table_column_edit = ['id','uid','address','house','landmark','lat','lng','title','pincode'];
    public $required = ['id'];
       ...
       $this->Address_model->saveUserLogs($saveLogInfo);
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Address_model->getById($_POST['id']);
                if($result != null){
    ...
    public function getByUid(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Address_model->getByUid($_POST['id']);
                if($result != null){
                    echo $this->json->response($result,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response($this->db->error(),$this->_Errmessage,$this->_statusErr);
    ...
    public function editList(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Address_model->editList($_POST,$_POST['id']);
    ...
    public function save(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Address_model->saveList($_POST);
                if($result != null){
                    $id = $this->db->insert_id();
                    $data = $this->Address_model->getByIdValue($id);
                    echo $this->json->response($data,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response(['error'=>'Something Went Wrong.'],$this->_Errmessage,$this->_statusErr);
                }
    ...
    public function deleteList(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Address_model->deleteList($_POST['id']);
                if($result != null){
                    echo $this->json->response($result,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response(['error'=>'Something Went Wrong.'],$this->_Errmessage,$this->_statusErr);
