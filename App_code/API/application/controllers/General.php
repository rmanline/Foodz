...
class General extends CI_Controller{
    ...
    public $_table_column_array = ['mobile','email','address','city','state','zip','country','min','free','tax','shipping','shippingPrice'];
    public $_table_column_edit = ['id','mobile','email','address','city','state','zip','country','min','free','tax','shipping','shippingPrice'];
    public $required = ['id'];
    ...
    public function save(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->General_model->saveList($_POST);
                if($result != null){
                    $id = $this->db->insert_id();
                    $data = $this->General_model->getByIdValue($id);
                    echo $this->json->response($data,$this->_OKmessage,$this->_statusOK);
                }else{
