...
class Banners extends CI_Controller{
    ...
    public $_table_column_array = ['cover','position','link','type','message','status'];
    public $_table_column_edit = ['id','cover','position','link','type','message','status'];
    public $required = ['id'];
    ...
    public function index()
	{
       ...
       $this->Banner_model->saveUserLogs($saveLogInfo);
       ...
       $this->Banner_model->saveUserLogs($saveLogInfo);
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Banner_model->getById($_POST['id']);
                if($result != null){
    ...
    public function editList(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Banner_model->editList($_POST,$_POST['id']);
    ...
    public function save(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Banner_model->saveList($_POST);
                if($result != null){
                    $id = $this->db->insert_id();
                    $data = $this->Banner_model->getByIdValue($id);
                    echo $this->json->response($data,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response(['error'=>'Something Went Wrong.'],$this->_Errmessage,$this->_statusErr);
                }
    ...
    public function deleteList(){
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Banner_model->deleteList($_POST['id']);
                if($result != null){
                    echo $this->json->response($result,$this->_OKmessage,$this->_statusOK);
                }else{
                    echo $this->json->response(['error'=>'Something Went Wrong.'],$this->_Errmessage,$this->_statusErr);
