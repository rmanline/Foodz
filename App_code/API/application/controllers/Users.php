...
class Users extends CI_Controller{
    ...
    public function validateUserToken(){
     ...
     public function validateStoreToken(){
        $token = $this->authorizations->getBearerToken();
    ...
    public function getDefaultSettings(){
       ...
       $this->Users_model->saveUserLogs($saveLogInfo);
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            $langName;
            if($haveFile){
                if(file_exists('uploads/'.$lang[0]->file)){
    ...
    public function adminHome(){
        ...
        if($tokenStatus == 200){
            ...
            $users = $this->Users_model->getCountsForAdmin();
            $orders = $this->Order_model->getAdminTop();
            $stores = $this->Stores_model->get_all();
            $allorders = $this->Order_model->ordersCountForAdmin();
       ...
       $this->Users_model->saveUserLogs($saveLogInfo);
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                if($data != null){
                    unset($data->password);
                    $token =$this->authorizations->generate($data->id,'3');
                    $modified = [
                        'token'=>$token['token'],
                        'data'=>$data,
                        'success'=>true,
       ...
       $this->Users_model->saveUserLogs($saveLogInfo);
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                if($data != null){
                    unset($data->password);
                    $token =$this->authorizations->generate($data->id,'2');
                    $modified = [
                        'token'=>$token['token'],
                        'data'=>$data,
                        'success'=>true,
        ...
        if($auth && $auth == $this->config->item('encryption_key')){
            ...
            if(isset($data) && !empty($data)){
                ...
                if($data != null && $data->type =='admin'){
                    ...
                    $token =$this->authorizations->generate($data->id,'1');
                    $modified = [
                        'token'=>$token['token'],
                        'success'=>true
                    ];
    ...
    public function loginWithPhoneAndPasswordUser(){
        $agent = $this->input->request_headers();
        $saveLogInfo = array(
            'url' => $this->uri->uri_string(),
            'agent' => json_encode($agent),
    ...
    public function loginWithPhoneAndPasswordStore(){
        $agent = $this->input->request_headers();
        $saveLogInfo = array(
            'url' => $this->uri->uri_string(),
            'agent' => json_encode($agent),
    ...
    public function getByIDAfterVerify(){
        $agent = $this->input->request_headers();
        $saveLogInfo = array(
            'url' => $this->uri->uri_string(),
            'agent' => json_encode($agent),
        ...
        if(isset($data) && !empty($data)){
            ...
            if($result != null){
                ...
                $token =$this->authorizations->generate($result[0]->id,'3');
                $modified = [
                    'token'=>$token['token'],
                    'data'=>$result,
                    'success'=>true,
    ...
    public function getByIDAfterVerifyStore(){
        $agent = $this->input->request_headers();
        $saveLogInfo = array(
            'url' => $this->uri->uri_string(),
            'agent' => json_encode($agent),
        ...
        if(isset($data) && !empty($data)){
            ...
            if($result != null){
                ...
                $token =$this->authorizations->generate($result[0]->id,'2');
                $modified = [
                    'token'=>$token['token'],
                    'data'=>$result,
                    'success'=>true,
                ...
                if($data != null){
                    ...
                    if($result != null){
                        ...
                        if($result != null){
                            ...
                            $token =$this->authorizations->generate($id,'3');
            ...
            if(isset($data) && !empty($data)){
                ...
                if($data != null){
                    ...
                    if($result != null){
                        ...
                        $result = $this->Users_model->save_users($_POST);
                        if($result != null){
                            $token =$this->authorizations->generate($this->db->insert_id(),'1');
                            $modified = [
                                'token'=>$token['token']
                            ];
                            echo $this->json->response($modified,$this->_OKmessage,$this->_statusOK);
