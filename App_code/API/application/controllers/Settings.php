...
class Settings extends CI_Controller{
    ...
    public $_table_column_array = ['currencySymbol','currencySide','appDirection','logo','twillo','creds','delivery','driver_login','store_login','user_login','web_login','reset_pwd','web_category'];
    public $_table_column_edit = ['id','currencySymbol','currencySide','appDirection','logo','twillo','creds','delivery','driver_login','store_login','user_login','web_login','reset_pwd','web_category'];
    public $required = ['id'];
    ...
    public function save(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Settings_model->saveList($_POST);
                if($result != null){
                    $id = $this->db->insert_id();
                    $data = $this->Settings_model->getByIdValue($id);
                    echo $this->json->response($data,$this->_OKmessage,$this->_statusOK);
                }else{
    ...
    public function editList(){
        ...
        if($tokenStatus == 200){
            ...
            if(isset($data) && !empty($data)){
                ...
                $result = $this->Settings_model->editList($_POST,$_POST['id']);
