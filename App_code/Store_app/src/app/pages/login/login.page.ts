...
export class LoginPage implements OnInit {
    ...
    login() {
        ...
        this.api.post('users/loginStore', param).subscribe((data: any) => {
            console.log(data);
            if (data && data.status === 200) {
                if (data && data.data && data.data.data.type === 'store') {
                    if (data.data.data.status === '1') {
                        ...
                        localStorage.setItem('name', data.data.data.first_name + ' ' + data.data.data.last_name);
                        localStorage.setItem('email', data.data.data.email);
                        localStorage.setItem('cover', data.data.data.cover);
                        const store = {
                        ...
                        Swal.fire({
                            ...
                            if (status && status.value) {
                                localStorage.setItem('helpId', data.data.data.id);
    ...
    onPhoneLogin() {
        ...
        this.loggedIn = true;
        this.api.post('users/loginWithPhoneAndPasswordStore', param).subscribe((data: any) => {
            this.loggedIn = false;
            console.log(data);
            if (data && data.status === 200) {
                if (data && data.data && data.data.data.type === 'store') {
    ...
    async openModal(uid) {
        ...
        modal.onDidDismiss().then((data) => {
            ...
            if (data && data.role === 'ok') {
                ...
                this.api.post('users/getByIDAfterVerifyStore', param).subscribe((data: any) => {
